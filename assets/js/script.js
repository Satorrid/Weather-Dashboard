...
function getCoord(q) {
  var url =
    "https://api.openweathermap.org/geo/1.0/direct?q=" + q + "&appid=" + APIKey;
  return fetch(url)
...
function updateDisplay(weather, day) {
  ...
  const forecastIcon = parentElement.querySelector("[forecast-icon]");
  forecastIcon.src = `https://openweathermap.org/img/wn/${weather.weather[0].icon}@2x.png`;
  const forecastDate = parentElement.querySelector("[forecast-date]");
  forecastDate.innerText = new Date(weather.dt * 1000).toLocaleDateString();
}
